  ...
  def create
    ...
    respond_to do |format|
      if @contact_information.save
        flash[:notice] = 'ContactInformation was successfully created.'
        format.html { redirect_to(@contact_information) }
  ...
  def update
    ...
    respond_to do |format|
      if @contact_information.update_attributes(params[:contact_information])
        flash[:notice] = 'ContactInformation was successfully updated.'
        format.html { redirect_to(@contact_information) }
        format.xml  { head :ok }
      else
